...
**团（Campaign）**:
一个命名的跑团记录容器，按团名 + 会话来源（origin）隔离；同团可跨多次开团
累积日志。摘要与查看按团定位，`开始记录 <团名>` 存在续写、不存在新建。
_Avoid_: 战役、跑团记录（记录指团内内容流，不是容器）

**场次（Session）**:
团内一次「开始记录 → 结束记录」的区间，团内第 N 次开团即第 N 场。摘要默认
最近一场，可指定场次。
_Avoid_: 局、次

**跑团日志（Session Log）**:
团内完整对话流（玩家消息 + 机器人回复）的逐条追加记录，存独立 SQLite
（`data_dir/trpg_log.db`，ADR-0029）；写入时对结算类规则预标。
_Avoid_: 战报（战报是摘要产物）、投掷历史（history 是另一概念）

**摘要 / 战报（Summary）**:
一场日志经 LLM 压缩成的叙事式剧情回顾 + 末尾结算统计；分类（行动/吐槽/结算）
是过滤手段而非交付物，结果落库为场次小结可回看。
_Avoid_: 总结（同义）

**行动（In-Character Action）**:
角色真正做的事——玩家扮演与 DM 旁白；摘要保留的主线。
_Avoid_: 剧情（剧情是行动的集合，非单条消息）

**场外吐槽（Out-of-Character Banter）**:
玩家场外的闲聊与插科打诨；摘要剔除，不进入剧情回顾。
_Avoid_: 闲聊、OOC

**结算（Mechanical Resolution）**:
掷骰指令与骰子结果；写入日志时由规则确定性预标，摘要折成结果（如「攻击命中，
造成 12 伤害」）。
_Avoid_: 掷骰（掷骰是结算的一部分）

**导入（Import）**:
把既有纯文本聊天记录（粘贴文本或上传 .txt/.log 文件）按行规则切分，落为团内
的一个**新场次**（不依赖记录状态，不打断正在 recording 的场次）；每行提取
「昵称: 内容」、骰式行预标结算。导入后即可用 `/总结` 生成摘要。
_Avoid_: 转录（转录是私设 manage_homebrew 的概念）
