...
class RollHistoryManager:
    ...
    def __init__(self, star: Star, max_count: int, enabled: bool = True) -> None:
        ...
        # 单把管理器级锁，保证 add() 的读-改-写与 clear() 的读-删互斥。
        # 曾经按会话 key 建立独立锁，但锁字典永不回收，长期运行会缓慢泄漏内存；
        # 历史操作本质只是两次 KV 调用，全局串行化带来的争用可忽略，
        # 换取实现简单与内存占用有界。
    ...
    async def add(
        ...
        try:
            ...
            # 使用管理器级锁保证 读-改-写 原子性，防止并发投掷（或与 clear()）互相覆盖。
            ...
            logger.warning(f"[trpg_assistant] 写入投掷历史失败: {e}")
        except Exception as e:
            # 兜底捕获 KV 存储可能抛出的未知异常，同样只记录警告。
            logger.warning(f"[trpg_assistant] 写入投掷历史时发生未预期异常: {e}")
    ...
    async def get_all(self, event: AstrMessageEvent) -> list[HistoryEntry]:
        ...
        try:
            ...
            logger.warning(f"[trpg_assistant] 读取投掷历史失败: {e}")
            return []
        except Exception as e:
            logger.warning(f"[trpg_assistant] 读取投掷历史时发生未预期异常: {e}")
            return []
    ...
    async def clear(self, event: AstrMessageEvent) -> int:
        ...
        try:
            key = _KV_PREFIX + event.unified_msg_origin
            # 与 add() 共用同一把锁：防止 clear() 的读-删与并发 add() 的读-写交错，
            # 否则可能出现"已清空的历史被在途 add() 复活"或新记录被静默丢弃。
            async with self._lock:
                raw = await self._star.get_kv_data(key, [])
            ...
            logger.warning(f"[trpg_assistant] 清空投掷历史失败: {e}")
            return 0
        except Exception as e:
            logger.warning(f"[trpg_assistant] 清空投掷历史时发生未预期异常: {e}")
            return 0
